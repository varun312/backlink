...
def main(stdscr):
    ...
    while True:
        ...
        if key.isdigit() and int(key) < len(newf):
            subprocess.run(["powershell","-Command","start '"+str(mainf[int(key)])+"'"])
        elif key == 'a':
            for x in range(len(newf)):
                subprocess.run(["powershell","-Command","start '"+str(mainf[x])+"'"])
        elif key == 'q':
            sys.exit()
